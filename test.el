...
(ert-deftest test-git-abl ()
  (let* ((base-dir (setup-git-tests))
    ...
    (should-error (find-file test-file-path))
    (commit-git base-dir)
...
(ert-deftest test-functionality ()
  (let ((vem-base (make-temp-file "blah" 't))
	(top-dir (make-temp-file "blah" 't))
	(project-dir (make-temp-file (concat-paths top-dir "web") 't)))
    (setq vems-base-dir vem-base)
    ))